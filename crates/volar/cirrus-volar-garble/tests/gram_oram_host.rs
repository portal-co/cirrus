...
fn encode_u64(
    ...
    v: u64,
    seed: u8,
) -> (Vec<volar_spec::garble::Eval<U16>>, Vec<Garble<U16>>) {
...
fn gram_oram_multiple_addrs() {
    ...
    let bits_of =
        |byte: u8| -> Vec<bool> { (0..(8 * B)).map(|i| (byte >> (i % 8)) & 1 == 1).collect() };
    let read_byte = |host: &mut GramOramHost<U16, Z, B>,
    ...
    assert_eq!(
        read_byte(&mut host, &mut tree, 0, &mut rng, 0x40),
        [0x11; B]
    );
    assert_eq!(
        read_byte(&mut host, &mut tree, 3, &mut rng, 0x60),
        [0x77; B]
    );
}
...
fn gram_oram_encrypted_tree_roundtrip() {
    ...
    let key = [0x42u8; 16];
    let mut host =
        GramOramHost::<U16, Z, B>::new_encrypted(det_secret(), levels, num_addrs, key, 16);
    let mut tree = OramTree::<Z, B>::new(levels);
    ...
    assert_eq!(
        plaintext_hits, 0,
        "encrypted tree must not store plaintext blocks"
    );
