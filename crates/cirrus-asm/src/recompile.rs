...
fn emit_call(w: &mut AArch64Writer, addr: usize, args: &[u64]) {
    ...
    // before each call.
    w.mov(&mut ctx, arch, &reg(BACKEND_ARG, MemorySize::_64), &reg(BACKEND_SAVE, MemorySize::_64))
        .unwrap();
...
pub fn compile_aarch64(program: &Program, pinned: &PinnedAddresses) -> Vec<u8> {
    ...
    .unwrap();
    w.mov(&mut ctx, arch, &reg(BACKEND_SAVE, MemorySize::_64), &reg(BACKEND_ARG, MemorySize::_64))
        .unwrap();
