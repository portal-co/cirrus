...
use cirrus_core::{ContextWithAdd, ContextWithSub, ContextWithValue, HasError};
pub struct Vole<Wrapped> {
    pub wrapped: Wrapped,
}
impl<Wrapped: HasError> HasError for Vole<Wrapped> {
...
impl<Wrapped: ContextWithAdd<Val>, Val> ContextWithAdd<Val> for Vole<Wrapped> {
    ...
    ) -> Result<<Self as cirrus_core::ContextWithValue<Val>>::Wrapped, Self::Error> {
        let (a0, a1) = a;
        let (b0, b1) = b;
        Ok((self.wrapped.add(a0, b0)?, self.wrapped.add(a1, b1)?))
    }
...
impl<Wrapped: ContextWithSub<Val>, Val> ContextWithSub<Val> for Vole<Wrapped> {
    fn sub(
        ...
        let (a0, a1) = a;
        let (b0, b1) = b;
        Ok((self.wrapped.sub(a0, b0)?, self.wrapped.sub(a1, b1)?))
    }
