//! The garbled-circuit garbler (`"gc"`) and evaluator (`"eval"`) pinned
//! backends, on top of the always-available [`plaintext`](crate::plaintext)
//! backend. Enabled by this crate's `gc` feature.
//!
//! [`GcBackend`] and [`EvalBackend`] wrap `cirrus-garbled-circuit`'s [`GC`]
//! and [`Evaluator`], adding the two operations a garbler/evaluator's own
//! API deliberately omits (`create`, `mux`) so each satisfies the same
//! [`cirrus_core`] `Context` trait bounds every pinned backend needs.
//! Neither wrapper changes `GC`/`Evaluator`'s own semantics; a caller who
//! wants direct access to the wrapped value can always reach it through the
//! public `gc`/`evaluator` field.
...
use cirrus_garbled_circuit::{Evaluator, EvaluationError, GarblingRecord, Label, GC};
use digest::Digest;

/// Byte width of a garbled-wire label this module's backends use.
pub const LABEL_BYTES: usize = 16;
/// The digest this module's backends derive fresh wire labels with.
...
/// [`define_pinned_backend!`](crate::define_pinned_backend)'s uniform bound
...
pub struct EvalBackend<'a, const N: usize> {
    ...
    pub evaluator: Evaluator<alloc_free::Boxed<'a, N>, N>,
}

/// A `Box<dyn Iterator<...>>` alias, spelled out once so
/// [`define_pinned_backend!`](crate::define_pinned_backend) (which needs a
/// plain `Ty<'lifetimes>` shape) has a single concrete generic parameter to
/// substitute rather than a raw `dyn` type.
pub mod alloc_free {
    use super::GarblingRecord;
    /// A boxed, type-erased garbling-record source.
    pub type Boxed<'a, const N: usize> = std::boxed::Box<dyn Iterator<Item = GarblingRecord<N>> + 'a>;
}

impl<'a, const N: usize> EvalBackend<'a, N> {
    /// Wrap an evaluator.
    pub fn new(evaluator: Evaluator<alloc_free::Boxed<'a, N>, N>) -> Self {
        Self { evaluator }
    }
}
...
crate::define_pinned_backend!(pub mod backend for ['a, 'b] GcBackend<'a, 'b, LabelDigest, LABEL_BYTES> as "gc");
crate::define_pinned_backend!(pub mod eval_backend for ['a] EvalBackend<'a, LABEL_BYTES> as "eval");

#[cfg(test)]
mod tests {
    use super::*;
    use cirrus_garbled_circuit::GarblingRecord;
    ...
    /// [`EvalBackend`] directly through the generic
    /// [`crate::execute`] runner (not yet through any code-generating
    /// backend -- that is `cirrus-recompile-tests`' job) -- the baseline
    /// sanity check that these two wrapper `Context`s round-trip correctly
    /// at all before any backend lowers a `Program` against them.
    #[test]
    fn garble_then_evaluate_round_trips_through_the_wrapper_backends() {
        ...
        let and = cirrus_core::ContextWithBitAnd::bitand(&mut recorder, a, b).unwrap();
        let or = cirrus_core::ContextWithBitOr::bitor(&mut recorder, a, b).unwrap();
        let xor = cirrus_core::ContextWithBitXor::bitxor(&mut recorder, a, b).unwrap();
        let mux = cirrus_core::ContextWithMux::mux(&mut recorder, a, one, zero).unwrap();
