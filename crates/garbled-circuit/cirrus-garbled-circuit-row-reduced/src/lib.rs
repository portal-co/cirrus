...
impl<'a, 'b, D: Digest, const N: usize> GC<'a, 'b, D, N> {
    ...
    fn output_zero_label(&self, left: [u8; N], right: [u8; N], gate: u64) -> [u8; N] {
        let row_zero_left = self.selected_for_row_zero(left);
        let row_zero_right = self.selected_for_row_zero(right);
        let mut output = self.row_zero_label(row_zero_left, row_zero_right, gate);
        let row_zero_value = left[0] & 1 == 0 && right[0] & 1 == 0;
        if row_zero_value {
    ...
    fn row(
        ...
        let left = (row & 1 != 0) ^ (left_zero[0] & 1 != 0) ^ (self.delta[0] & 1 != 0);
        let right = (row & 2 != 0) ^ (right_zero[0] & 1 != 0) ^ (self.delta[0] & 1 != 0);
        if left & right {
...
mod tests {
    ...
    use cirrus_core::{ContextWithBitAnd, Pusher};
    ...
    fn garbler_hash(_: &[[[u8; 16]; 32]]) -> Result<[u8; 32], Infallible> {
        Ok([0; 32])
    }
    ...
    fn evaluator_replays_all_and_truth_table_rows_from_three_row_records() {
        ...
        let left_zero = [0; 16];
        let right_zero = [2; 16];
        let result_zero: [[u8; 16]; 4] = array::from_fn(|_| {
            garbler
                .bitand(left_zero, right_zero)
                ...
                .zip(result_zero)
        {
            assert_eq!(
                ...
                label(result_zero, left & right),
                "left={left}, right={right}",
            );
        }
        assert_eq!(
            evaluator.bitand(left_zero, right_zero),
    ...
    fn evaluator_replays_an_ert_add_from_three_row_records() {
        ...
        garbled_registers[Reg::A1.0 as usize] = [zero; 32];
        garbled_registers[Reg::A2.0 as usize] = [right_zero; 32];
        ...
        for bit in 0..32 {
            assert_eq!(
                ...
                label(garbled_result[bit], (result >> bit) & 1 != 0),
                "result bit {bit}",
            );
        }
        assert_eq!(
